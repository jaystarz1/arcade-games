...
var MemStorage = class {
  ...
  async getHighScores(limit = 10) {
    return this.highScores.sort((a, b) => b.score - a.score).slice(0, limit);
  }
  async createHighScore(insertHighScore) {
    const highScore = {
...
var PostgresStorage = class {
  ...
  async getHighScores(limit = 10) {
    if (!db) throw new Error("Database not initialized");
    const result = await db.select().from(highScores).orderBy(desc(highScores.score)).limit(limit);
    return result;
  }
  async createHighScore(insertHighScore) {
    if (!db) throw new Error("Database not initialized");
...
async function registerRoutes(app2) {
  app2.get("/api/high-scores", async (req, res) => {
    try {
      const limit = parseInt(req.query.limit) || 10;
      const highScores2 = await storage.getHighScores(limit);
...
var vite_config_default = defineConfig({
  ...
  base: "/games/AsteroidsArcade/dist/public/",
  build: {
